...
export BAO_OS=${BAO_OS:-'Linux'}
export BAO_CPUARCH=${BAO_CPUARCH:-'x86_64'}
export BAO_VERSION=${BAO_VERSION:-'2.2.0'}

export BAO_BIN_DWNLD_LINK="https://github.com/openbao/openbao/releases/download/v${BAO_VERSION}/bao_${BAO_VERSION}_${BAO_OS}_${BAO_CPUARCH}.tar.gz"
export BAO_BIN_DWNLD_FILENAME=$(echo "${BAO_BIN_DWNLD_LINK}" | awk -F '/' '{ print $NF }')

export BAO_BIN_GPG_SIG_DWNLD_LINK="https://github.com/openbao/openbao/releases/download/v${BAO_VERSION}/bao_${BAO_VERSION}_${BAO_OS}_${BAO_CPUARCH}.tar.gz.gpgsig"
export BAO_BIN_GPG_SIG_FILENAME=$(echo "${BAO_BIN_GPG_SIG_DWNLD_LINK}" | awk -F '/' '{ print $NF }')

export BAO_BIN_COSIGN_SIG_DWNLD_LINK="https://github.com/openbao/openbao/releases/download/v${BAO_VERSION}/bao_${BAO_VERSION}_${BAO_OS}_${BAO_CPUARCH}.tar.gz.sig"
export BAO_BIN_COSIGN_SIG_FILENAME=$(echo "${BAO_BIN_COSIGN_SIG_DWNLD_LINK}" | awk -F '/' '{ print $NF }')

export BAO_BIN_CERT_DWNLD_LINK="https://github.com/openbao/openbao/releases/download/v${BAO_VERSION}/bao_${BAO_VERSION}_${BAO_OS}_${BAO_CPUARCH}.tar.gz.pem"
...
sudo mkdir -p /usr/bin/bao-${BAO_VERSION}/
sudo mv ./deflated/bao /usr/bin/bao-${BAO_VERSION}/
sudo mv ./deflated/LICENSE  /usr/bin/bao-${BAO_VERSION}/
sudo mv ./deflated/*.md  /usr/bin/bao-${BAO_VERSION}/

sudo ln -s /usr/bin/bao-${BAO_VERSION}/bao /usr/bin/bao
